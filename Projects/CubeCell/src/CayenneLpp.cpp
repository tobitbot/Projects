...
uint8_t CayenneLPP::getTypeSize(uint8_t type)
{
  ...
  case CUSTOM_ECO2:
    return CUSTOM_ECO2_SIZE;

  case CUSTOM_TVOC:
    return CUSTOM_TVOC_SIZE;
...
uint8_t CayenneLPP::addTVOC(uint8_t channel, uint16_t value)
{
  return addField(CUSTOM_TVOC, channel, value);
}

uint8_t CayenneLPP::addEco2(uint8_t channel, uint16_t value)
{
  return addField(CUSTOM_ECO2, channel, value);
}
